...
texts = {
    "ar": {
        "title": "حاسبة التعرفة للمنازل والماء والبترول – البحرين",
        "electricity": "⚡ التعرفة الكهربائية للمنزل",
        "water": "💧 التعرفة المائية للمنزل",
        "fuel": "⛽ تعرفة الوقود",
        "enter_bill": "أدخل قيمة الفاتورة",
...
# ---------------------------
with tab1:
    input_type = st.radio(t["enter_bill"], [t["enter_bill"], t["enter_usage"]], horizontal=True)
    value = st.number_input("", min_value=0.0, step=1.0)

    slabs = [
        (3000, 0.003, 0.003),       # 0-3000
        (2000, 0.009, 0.009),       # 3001-5000
        (float("inf"), 0.016, 0.032) # >5000
    ]
    slab_names = ["الشريحة الأولى","الشريحة الثانية","الشريحة الثالثة"]
    ...
    if value>0:
        ...
        fig=go.Figure()
        for i in range(3):
            fig.add_bar(y=["الاستهلاك"], x=[usage[i]], orientation="h",
                        name=slab_names[i], marker_color=colors[i])
        fig.update_layout(barmode="stack", height=260, xaxis_title="kWh", yaxis_visible=False)
...
# ---------------------------
with tab2:
    input_type = st.radio(t["enter_bill"], [t["enter_bill"], t["enter_usage"]], horizontal=True, key="water_radio")
    value = st.number_input("", min_value=0.0, step=1.0, key="water_input")
    ...
    if value>0:
        ...
        for i in range(3):
            fig.add_bar(y=["الاستهلاك"], x=[usage[i]], orientation="h",
                        name=slab_names[i], marker_color=colors[i])
        fig.update_layout(barmode="stack", height=260, xaxis_title="م³", yaxis_visible=False)
        st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    for index, row in fuel_prices.iterrows():
        liters = st.number_input(f"{row['fuel']} – عدد اللترات", min_value=0.0, step=1.0, key=f"fuel_{index}")
